...
def make_repeater(func, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def repeat_repeater(a):
        k = 1
        while k <= n:
            a, k = func(a), k + 1
        return a
    return repeat_repeater
...
def missing_digits(n):
    ...
    if n <= 9:
        ...
        if last_digit - next_last_digit > 0:
            return missing_digits(n // 10) + last_digit - next_last_digit - 1
        else:
...
def count_coins(change):
    ...
    def count_helper(change, smallest_coin):
        if change < 0 or smallest_coin == None:
            return 0
        elif change == 0:
            return 1
        else:
