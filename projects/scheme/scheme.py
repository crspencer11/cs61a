...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(f)
        r = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(f, r, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        new_expression = scheme_eval(expressions.first, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lamb = do_lambda_form(Pair(formals, body), env)
...
def do_and_form(expressions, env):
    ...
    if not expressions:
        ...
        while expressions is not nil:
            ...
            if is_false_primitive(value) == True:
                return False
            expressions = expressions.rest            
        return value
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    #     names = validate_formals(bindings.first)
    #     values = validate_form(scheme_eval(bindings.rest, env), 2, 2)
    while bindings is not nil:
        nav = bindings.first
        validate_form(nav, 2, 2)
        names = Pair(nav.first, names)
        values = Pair(scheme_eval(nav.rest.first, env), values)
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, n, env):
        return env.make_child_frame(self.formals, n)       
    # END PROBLEM 15
