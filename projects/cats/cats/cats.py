...
def choose(paragraphs, select, k):
    ...
    for paragraph in paragraphs:
        if select(paragraph):
            condition_met.append(paragraph)
    if k >= len(condition_met):
        return ''
    return condition_met[k]
...
def accuracy(typed, reference):
    ...
    length_reference = len(reference_words)
    if length_typed == 0 and length_reference == 0:
        return 100.0
    elif length_typed == 0 and length_reference > 0:
        return 0.0
    else:
        i = 0
        while i < length_typed and i < length_reference:
            if reference_words[i] == typed_words[i]:
                total += 1
            i += 1
    return total / length_typed * 100
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    min_dif = diff_function(typed_word, valid_words[0],limit)
    index = 0
    for i in range(len(valid_words)):
        if diff_function(typed_word, valid_words[i],limit) < min_dif:
            min_dif = diff_function(typed_word, valid_words[i], limit)
            index = i
    if min_dif > limit:
        return typed_word
...
def report_progress(sofar, prompt, user_id, send):
    ...
    current_i = 0
    if prompt == sofar:
        send({'id': user_id, 'progress': 100.0})
        return 100.0
    else:
...
def fastest_words(game):
    ...
    for player in player_indices:
        fastest_player_list.append([])

    for word in word_indices:
        fastest_time = get_times(game)[0][word]
        fastest_player = 0
        for player in player_indices:
            if get_times(game)[player][word] < fastest_time:
                fastest_time = get_times(game)[player][word]
                fastest_player = player
        fastest_player_list[fastest_player].append(word_at(game, word))
    return fastest_player_list
    # END PROBLEM 10
